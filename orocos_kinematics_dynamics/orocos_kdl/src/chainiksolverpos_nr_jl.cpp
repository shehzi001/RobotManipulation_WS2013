...
namespace KDL
{
    ...
    int ChainIkSolverPos_NR_JL::CartToJnt(const JntArray& q_init, const Frame& p_in, JntArray& q_out)
    {
            ...
            //std::cout << "delta twist:[" << delta_twist.vel.data[0] << "," << delta_twist.vel.data[1] << "," << delta_twist.vel.data[2] << "]"<< std::endl;
				if(Equal(delta_twist,Twist::Zero(),eps))
					break;
            
				iksolver.CartToJnt(q_out,delta_twist,delta_q);
                Add(q_out,delta_q,q_out);
           //std::cout << "joint angle = [" << q_out(0) << "," << q_out(1) << "," << q_out(2) << "," << q_out(3) << "," << q_out(4) << "]" << std::endl;
